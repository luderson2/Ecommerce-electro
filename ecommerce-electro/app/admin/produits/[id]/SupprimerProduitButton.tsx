...
export default function SupprimerProduitButton({ id, nom }: { id: string; nom: string }) {
  const [state, formAction, isPending] = useActionState(supprimerProduit, null);
  ...
  return (
    <form
      action={formAction}
      onSubmit={(e) => {
        if (!confirm(`Supprimer le produit "${nom}" ?`)) e.preventDefault();
      }}
    >
