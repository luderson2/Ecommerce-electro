...
const STATUTS: { value: SavStatus | "tous"; label: string }[] = [
...
export default async function AdminSavPage({
  ...
  let query = supabase
    ...
    .select("id, subject, status, created_at, order_id, profiles(full_name)")
  ...
  const { data: demandes, error } = await query as unknown as {
    data: DemandeLigne[] | null;
                ...
                <tr key={demande.id} className="hover:bg-surface/60 transition-colors">
                  ...
                  <td className="p-0">
                    <Link href={`/admin/sav/${demande.id}`} className="flex items-center px-5 py-3 text-muted-foreground">
                      {demande.profiles?.full_name ?? <span className="italic">Inconnu</span>}
                    </Link>
                  </td>
